...
Item *onList(list_t *list,
             char *board);     // return an item with same board , or null
Item *popFirst(list_t *list);  // return and remove first item
Item *popLast(list_t *list);   // return and remove last item
Item *popBest(
    list_t *list);  // return and remove best item with minimal f value
...
void printList(list_t list);              // affiche liste
